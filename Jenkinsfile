...
usernameVariable: 'DOCKER_USER', passwordVariable: 'DOCKER_PASSWORD']]) {
                sh """
                    ...
                    docker run -i --network=curltest --link=dropw-test tutum/curl /bin/bash -c 'ping google.com && ping dropw-test && /usr/bin/curl --retry 10 --retry-delay 5 -v http://dropw-test/hello-world'
